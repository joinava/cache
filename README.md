...
- [`wrapComputingProducer.ts`](./src/utils/wrapComputingProducer.ts): variants of `wrapProducer`/`wrapBulkProducer` (`wrapComputingProducer` and `wrapBulkComputingProducer`) for the case where the cached value is an expensive-to-compute **function of some input**, rather than a lookup of a mutable entity by id — e.g. an LLM extraction over a chunk of text. You provide a `hashInput` function and a producer that takes the full input; the wrapper derives the cache id from the hash, keeps the input around (reference-counted, so it survives request collapsing without leaking) just long enough to hand it to the producer on a miss, and otherwise behaves exactly like `wrapProducer`/`wrapBulkProducer`. See the file's module doc for the "lookup vs. compute" distinction.
