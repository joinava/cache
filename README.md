...
A cache's unit of storage is the `(id, vary)` **slot**, not the id: a producer that varies on request params writes one entry per variant under one id. So the three channels that can name a slot all report the same normalized `vary` object — `store-entry` for the entry written, `read`/`fetch` for the entry a lookup *selected* — which is what lets a subscriber attribute serves to the exact stored value (e.g. how many times one version was served before a newer one replaced it) instead of smearing every variant's serves onto whichever variant stored first. Where nothing was served out of a slot the key is **omitted**, never published as `{}`, because `{}` is a real slot — the one a non-varying producer writes. That covers `found: "none"`/`"read-failed"` on `read`, and `producer-error`/`aborted` on `fetch` — and also `served-from-producer`, which did hand the caller a value: that value came from the origin, not a slot, and the slot it lands in is reported on `store-entry` if and when the write succeeds.
